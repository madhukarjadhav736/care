...
from care.facility.models.patient import PatientNotes
...
class FileUploadViewSet(
    ...
    def get_queryset(self):
        ...
        if (
            "consultation_id" in self.request.GET
            and file_type == FileUpload.FileType.PATIENT_NOTES
        ):
            consultation_id = self.request.GET["consultation_id"]
            patient_notes = PatientNotes.objects.filter(
                consultation__external_id=consultation_id
            ).only("external_id")
            associating_ids = patient_notes.values_list("external_id", flat=True)
        else:
